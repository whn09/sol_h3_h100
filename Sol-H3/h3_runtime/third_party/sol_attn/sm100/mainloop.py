    ...
    while logical_group_idx < num_logical_groups:
        ...
        for half_idx in cutlass.range(
            ...
            if is_owner:
                ...
                if has_route_approx:
                    ...
                    local_max = Float32(local_max) * softmax_scale_log2
        ...
        if is_owner and has_pair_exact:
            ...
            for pair_idx in cutlass.range(pair_count, unroll=1):
                ...
                # Nearly every exact pair covers two complete N64 blocks.  In
                # that common case all column gates are +0 and need neither a
                # shared-memory round trip nor an add to every score.  Tail
                # pairs retain the original mask construction exactly.
                all_columns_valid = (
                    valid0 == Int32(N_MEMBER)
                    and valid1 == Int32(N_MEMBER)
                )
                if exact_owner_warp == Int32(0) and not all_columns_valid:
                ...
                if not all_columns_valid:
                    ...
                    # Full columns still require the original row gate on the
                    # final partial query block.
                    row_mask = -Float32.inf
                    for i in cutlass.range_constexpr(
                        0, cute.size(pair_scores), 2
                    ):
                        score0, score1 = cute.arch.add_packed_f32x2(
                            (
                                Float32(pair_scores[i]),
                                Float32(pair_scores[i + 1]),
                            ),
                        ...
                        pair_scores[i] = score0
                        pair_scores[i + 1] = score1
