...
def _online_update_one_half(
    ...
    """Apply one FP32 online-softmax update with maxima stored in base-2 units."""
    ...
    if running_max != -Float32.inf:
        alpha = cute.math.exp2(running_max - new_max, fastmath=True)
    ...
    for i in cutlass.range(cute.size(scores), unroll_full=True):
        probabilities[i] = cute.math.exp2(
            Float32(scores[i]) * softmax_scale_log2 - new_max,
            fastmath=True,
        )
